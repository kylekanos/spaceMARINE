module problem_module
 ...
 contains
   ...
   subroutine init_phi_2d(phi, ng, lo, hi, prob_lo, nvars, dx)
     ...
     do j=lo(2),hi(2)
        do i=lo(1),hi(1)
           x = CellPos2D(prob_lo, dx, i, j)
           r2 = dot_product(x,x)
           
           phi(i,j,1) = 1.0_dp_t
           phi(i,j,2:4) = 0.0_dp_t
           if(r2 < 0.1_dp_t) then
              phi(i,j,5) = 10.0*gamma7
           else
              phi(i,j,5) = 0.1*gamma7
           endif
        enddo !- i
     enddo !- j
     !$omp end parallel do
     ...
     do k=lo(3),hi(3)
        do j=lo(2),hi(2)
           do i=lo(1),hi(1)
              x = CellPos3D(prob_lo, dx, i, j, k)
              r2 = dot_product(x,x)
   
              phi(i,j,k,1) = 1.0_dp_t
              phi(i,j,k,2:4) = 0.0_dp_t
              if(r2 < 0.1_dp_t) then
                 phi(i,j,k,5) = 10.0*gamma7
              else
                 phi(i,j,k,5) = 0.1*gamma7
              endif
           enddo !- i
        enddo !- j
     enddo !- k
